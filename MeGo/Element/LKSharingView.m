...
@interface LKSharingView ()<WXApiDelegate>
...
- (void)shareWithType:(int)scene
{
    ...
    [img sd_setImageWithURL:[NSURL URLWithString:self.store.s_photo_url]];
    UIImage *im = img.image;
    [message setThumbImage:im];
    WXWebpageObject *web = [WXWebpageObject object];
    web.webpageUrl = self.store.business_url;
    message.mediaObject = web;
    message.title = self.store.name;
    message.description = [NSString stringWithFormat:@"您的小伙伴在MeGo中发现了%@~",self.store.name];
